...
def observation(
    at_seconds: int, *, hud: bool = False, result: bool = False
) -> CoarseObservation:
    return CoarseObservation(
        at_ms=at_seconds * 1_000,
        hud_signature='lineup' if hud else None,
...
def test_searches_after_each_gameplay_run_and_at_video_end() -> None:
    ...
    assert windows == (ScanWindow(0, 70_000), ScanWindow(85_000, 110_000))
...
def test_temporary_scoreboard_gap_merges_into_one_result_window() -> None:
    ...
    assert windows == (ScanWindow(95_000, 180_000),)
