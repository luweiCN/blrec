...
| 审核列表/稿件详情 | `/x/web/archives`、`/x/vupre/web/archive/view` | 当前版本 Web Cookie jar |
...
`ReviewWatcher` 每 15 分钟按账号读取近期审核列表。列表只负责匹配 AID/BVID 和审核状态；仅在稿件通过时读取一次投稿详情，以远端 filename 绑定 CID。写入 CID 前校验账号作用域、AID/BVID、远端 filename、分 P 数量和页码；不能只按返回数组位置匹配。审核拒绝保存原因并停止后处理。审核通过后并列创建评论和回灌两个独立子工作流；`AccountWriteGate` 优先尝试评论，但评论失败不会阻塞回灌。
