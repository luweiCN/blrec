...
Migration 28 新增 `upload_retry_batches(operation_id,state,total_items,created_at,updated_at)` 与 `upload_retry_batch_items(operation_id,job_id,state,error_code)`，并建立 `(operation_id,state,job_id)` 索引。route 先生成 operation ID，在**同一个 upload DB transaction** 写 batch，并用一次 set-based INSERT 冻结提交瞬间的完整 membership：
