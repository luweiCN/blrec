...
NAS Server 持久化登记 Worker ID、显示名称、是否允许领取、最后心跳、模型包、
管线版本、并发量以及完成／失败／累计处理耗时。新 Worker 可用 `--worker-id` 或
`BLREC_ANALYSIS_WORKER_ID` 指定稳定 ID；未预先登记的旧节点首次领取时自动登记。

每台 Worker 还会按成功完成的完整视频任务累计两项加权效率：核心解码分析耗时和
从领取到结果落库的整体任务耗时，统一显示为“分钟／1 小时视频”。分母使用累计
视频时长，不对长短视频做简单平均；失败任务、单局重识别和头像补标不进入效率
样本。旧任务缺少 Worker 归属和分阶段耗时，不能可靠回填，因此从支持该协议的
Worker 上线后开始累计。

管理端“安全暂停”只禁止该节点领取新任务，不中断已经领取的任务；暂停状态写入
数据库，Server 重启后仍有效。暂停更新和任务领取使用同一把服务端锁，避免操作
返回成功时另一个并发领取请求又拿到新任务。手动添加只预登记 ID 和显示名称，
不会保存 SSH 凭据，也不会负责远程安装或启动节点。
...
部署候选；生成后可在同一页面点“部署到 Worker”。Vision Lab 通过 SSH 把完整包
上传到 MacBook Pro 的版本目录，远端再次校验清单和文件哈希，再原子切换
`current` 并重新加载 launchd。页面只有收到 Worker 稳定启动结果后才显示部署
成功；失败时自动恢复原 launchd 配置和原模型包。

### 回滚

- 回滚单位是完整模型包或对应 Analysis Worker 发布版本，不是手工复制某个旧 ONNX。
- 发布前保留当前包和 Worker 安装包。
- 可在模型测试页对以前的完整模型包再次点击“部署此版本”；这会走同一套校验、原子切换和启动检查，而不是覆盖单个 ONNX。
- 新模型启动失败时发布程序自动恢复旧版本；NAS 未完成任务会按现有心跳租约重新派发。
