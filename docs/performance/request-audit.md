...
| I-088 | GET | `/api/v1/highlights/sessions/{session_id}/timeline` | highlights | R | D100 | `src/blrec/web/routers/highlights.py:get_timeline` -> active durations plus full timeline | Full marker/part timeline is loaded where row views only need counts. | Hot read: add lightweight counts; keep full timeline lazy in editor. |
...
| I-092 | GET | `/api/v1/highlights/clips` | highlights | R,F | L150 | `src/blrec/web/routers/highlights.py:list_all_clips` -> count plus full clip list | Paged list may hydrate sources/upload summaries and file metadata per item. | Hot read: bulk summaries, constant SQL, zero per-row `stat`, matching indexes. |
...
| I-099 | POST | `/api/v1/highlights/clips/{clip_id}/upload-task` | highlights | R,W | C100 | `src/blrec/web/routers/highlights.py:create_upload_task` -> local upload-job creation | Expensive remote upload remains worker-owned. | Keep; no synchronous remote upload. |
