...
| I-011 | POST | `/api/v1/tasks/actions` | tasks | W,F,X | C100 local; EXT remote | `src/blrec/web/routers/tasks.py:run_task_batch_action` -> `TaskControlReconciler.submit` for lifecycle actions; direct handlers for refresh/cut/delete | Start/stop/recorder actions return 202 after one durable admission and at most one desired-state dump; refresh/cut/delete retain their existing synchronous semantics. | Write/media WM-06 implemented for lifecycle actions with one `task-state` worker; remote refresh remains Outbound work. |
...
| I-022 | POST | `/api/v1/tasks/start` | tasks | W,F,X | C100 local; EXT remote | `src/blrec/web/routers/tasks.py:start_all_tasks` -> `_submit_control` -> `TaskControlReconciler.submit` | Returns 202 with per-room admission after one desired-state dump; a durable lane-1 worker performs lifecycle work. | Write/media WM-06 implemented; Outbound still owns reuse of remote room/play results inside the worker. |
| I-023 | POST | `/api/v1/tasks/{room_id}/start` | tasks | W,F,X | C100 local; EXT remote | `src/blrec/web/routers/tasks.py:start_task` -> `_submit_control` -> `TaskControlReconciler.submit` | Returns 202 without waiting for refresh, monitor, recorder, or stream setup; terminal state is read through I-107. | Write/media WM-06 implemented; Outbound still owns shared detail/play resolution. |
| I-024 | POST | `/api/v1/tasks/stop` | tasks | W,F | C100 ack; local batch <2 s | `src/blrec/web/routers/tasks.py:stop_all_tasks` -> `_submit_control` -> `TaskControlReconciler.submit` | One batch admission and at most one settings dump precede serialized, restart-recoverable lifecycle convergence. | Write/media WM-06 implemented with durable per-room steps and one `task-state` worker. |
| I-025 | POST | `/api/v1/tasks/{room_id}/stop` | tasks | W,F | C100 ack | `src/blrec/web/routers/tasks.py:stop_task` -> `_submit_control` -> `TaskControlReconciler.submit` | Returns 202; recorder finalization is worker-owned and force affects only this convergence attempt. | Write/media WM-06 implemented; status and stable errors are exposed by I-107. |
| I-026 | POST | `/api/v1/tasks/recorder/enable` | tasks | W,X | C100 ack; EXT remote | `src/blrec/web/routers/tasks.py:enable_all_task_recorders` -> `_submit_control` -> `TaskControlReconciler.submit` | One desired-state dump admits the batch; no-op batches perform zero settings writes and remote stream setup runs in the worker. | Write/media WM-06 implemented; Outbound still owns play-result reuse. |
| I-027 | POST | `/api/v1/tasks/{room_id}/recorder/enable` | tasks | W,X | C100 ack; EXT remote | `src/blrec/web/routers/tasks.py:enable_task_recorder` -> `_submit_control` -> `TaskControlReconciler.submit` | Returns 202 before recorder/stream setup and reports the terminal per-room result through I-107. | Write/media WM-06 implemented; Outbound still owns shared play resolution. |
| I-028 | POST | `/api/v1/tasks/recorder/disable` | tasks | W,F | C100 ack; local batch <2 s | `src/blrec/web/routers/tasks.py:disable_all_task_recorders` -> `_submit_control` -> `TaskControlReconciler.submit` | One batch admission and at most one desired-state dump replace repeated foreground lifecycle work. | Write/media WM-06 implemented with durable serialized convergence. |
| I-029 | POST | `/api/v1/tasks/{room_id}/recorder/disable` | tasks | W,F | C100 ack | `src/blrec/web/routers/tasks.py:disable_task_recorder` -> `_submit_control` -> `TaskControlReconciler.submit` | Returns 202; finalization and optional force run in the durable worker, with terminal status through I-107. | Write/media WM-06 implemented. |
...
| ID | Method | Normalized path | Router | IO | Budget | Evidence | Finding | Disposition |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| I-107 | GET | `/api/v1/control-operations/{operation_id}` | control_operations | R | D100 | `src/blrec/web/routers/control_operations.py:get_control_operation` -> dedicated control journal | Reads one durable operation and its bounded per-item steps without running lifecycle work. | Write/media WM-06 implemented: cancellable client polling follows accepted task controls to a terminal result; I-106 is the WM-05 inspection endpoint. |
