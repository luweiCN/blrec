...
- “云成本”汇总阿里云账号账单、`vg.luwei.host` 的 CDN 用量、OSS 当前
  Bucket 占用及计费用量，并支持切换近 18 个月账期和查看每日账单。
...
- 月/日账单、OSS 计费项和指定 Bucket 当前容量查询；
- CDN 域名用量查询；
...
- OSS 的“当前总占用”和对象数来自 `luwei-vainglory` 的 Bucket 统计，数据通常延迟一小时以上；
  计费用量与费用来自账号产品级月账单，阿里云账单接口不能把费用精确拆分到单个 Bucket。
- OSS 容量按小时出账。账单中的存储 `Usage` 是账期内每小时容量的累计量，例如持续存储
  `1 GB` 24 小时会得到 `24 GB·小时`，它不是当前占用量；页面直接展示账单返回的目录单价和单位，
  并根据返回单位换算该计量周期的平均占用，作为辅助理解。
- 产品费用中的块存储（控制台也可能显示为云盘或类似名称）属于 ECS 使用的块设备，不是 OSS；
  页面保留产品 Code 并在名称下说明用途，便于追查实际账单来源。
- 账单数据相对实际用量通常延迟约 24 小时，当前月账单也可能继续调整；页面会标出生成时间并使用 10 分钟缓存。
- 日账单按所选月份逐日查询。查询会串行执行以遵守 BSS OpenAPI 的单用户限流，首次切换月份可能比缓存命中慢。
...
- Logstore 原始日志仍只保留 7 天。BLREC 启动后每 5 分钟把可用于分析的访问明细增量归档到 NAS 上的主数据库，首次启动会尽量补回当时仍可查询的最近 7 天；更早且已经从 SLS 过期的原始明细无法追回。
- 本地归档使用请求 UUID 去重，长期保存事件时间、页面、来源、粗粒度设备/浏览器以及由 SLS 解析出的地域和运营商；匿名访客 ID 会再次单向哈希，不保存原始 IP 和完整 User-Agent。主数据库备份会一并保护这些历史数据。
- 首次同步完成后，“访问分析”可以查询 7 天以上的范围。页面会分别显示 SLS 原始日志保留期、本地归档起始时间和已同步到的时间；同步中断后会从上次进度重叠补扫并自动去重。

## 验收

1. 先在网络管理页确认两个用途均为“固定线路”。
2. 打开“云成本”，确认月账单、日账单、CDN 用量、OSS 当前占用和计费明细返回；若为部分结果，
   按页面警告补 `bss:QueryAccountBill`、`oss:GetBucketStat` 等 RAM 权限。
...
6. 等待首次归档完成，选择“最近 30 天”或“全部”，确认页面显示本地归档起止时间；检查 `visitor_analytics_sync_state` 没有同步错误。
