...
CREATE TABLE players (
    ...
    initial TEXT NOT NULL CHECK (
        initial=btrim(initial) AND length(initial) BETWEEN 1 AND 4
    ),
...
CREATE TABLE player_aliases (
    ...
    alias TEXT NOT NULL CHECK (
        alias=btrim(alias) AND length(alias) BETWEEN 1 AND 80
    ),
    PRIMARY KEY (player_id,alias)
);

CREATE UNIQUE INDEX player_aliases_casefold_idx
ON player_aliases(player_id,lower(alias));
...
CREATE TABLE matches (
    ...
    season_key TEXT NOT NULL CHECK (
        season_key ~ '^[0-9]{4}-(spring|summer|autumn)$'
    ),
...
CREATE TABLE match_participants (
    ...
    is_recorded_player SMALLINT NOT NULL CHECK (is_recorded_player IN (0,1)),
...
ON match_participants(lower(hero_name),match_id);

CREATE INDEX match_participants_player_name_idx
ON match_participants(lower(player_name),match_id);
...
CREATE TABLE rating_events (
    ...
    ability_after DOUBLE PRECISION NOT NULL CHECK (
        ability_after > 0 AND ability_after < 1
    ),
    evidence_after DOUBLE PRECISION NOT NULL CHECK (evidence_after > 0),
    provisional SMALLINT NOT NULL CHECK (provisional IN (0,1)),
...
CREATE TABLE match_search (
    match_id BIGINT NOT NULL REFERENCES matches(source_match_id) ON DELETE CASCADE,
...
CREATE INDEX match_search_match_idx ON match_search(match_id);
CREATE INDEX match_search_normalized_trgm_idx
ON match_search USING gin(normalized gin_trgm_ops);
CREATE INDEX match_search_pinyin_trgm_idx
ON match_search USING gin(pinyin gin_trgm_ops);
CREATE INDEX match_search_initials_trgm_idx
ON match_search USING gin(initials gin_trgm_ops);
