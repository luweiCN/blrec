...
class IngestPlayer(StrictModel):
    ...
    public_visible: bool = Field(default=True, alias='publicVisible')
...
class IngestMatch(StrictModel):
    ...
    season_key: str = Field(
        alias='seasonKey', regex=r'^\d{4}-(spring|summer|autumn|winter)$'
    )
    ...
    duplicate_review_state: Literal['none', 'pending', 'confirmed', 'dismissed'] = (
        Field(default='none', alias='duplicateReviewState')
    )
    ally: IngestMatchTeam
    enemy: IngestMatchTeam
    replay: Optional[IngestReplay] = None
    replay_access: Literal['public', 'owner'] = Field(
        default='public', alias='replayAccess'
    )
...
class IngestBatch(StrictModel):
    ...
    source_revision: int = Field(alias='sourceRevision', gt=0)
    publish: bool = True
    reset: bool = False
    generated_at: datetime = Field(alias='generatedAt')
    source_last_match_id: int = Field(alias='sourceLastMatchId', ge=0)
    players: List[IngestPlayer] = Field(max_items=500)
    matches: List[IngestMatch] = Field(max_items=500)
    removed_match_ids: List[int] = Field(alias='removedMatchIds', max_items=2000)
