...
CREATE TABLE dashboard_cache_players (
    ...
    player_json TEXT NOT NULL CHECK (
        json_valid(player_json) AND json_type(player_json)='object'
    ),
...
CREATE TABLE dashboard_cache_matches (
    ...
    season_key TEXT NOT NULL CHECK (
        length(season_key)=11
        AND substr(season_key,1,4) GLOB '[0-9][0-9][0-9][0-9]'
        AND substr(season_key,5,1)='-'
        AND substr(season_key,6) IN ('spring','summer','autumn','winter')
    ),
    ...
    has_replay INTEGER NOT NULL CHECK (has_replay IN (0,1)),
    match_json TEXT NOT NULL CHECK (
        json_valid(match_json) AND json_type(match_json)='object'
    ),
    ratings_json TEXT NOT NULL CHECK (
        json_valid(ratings_json) AND json_type(ratings_json)='object'
    ),
