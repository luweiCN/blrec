...
from .normalized_dashboard import refresh_dashboard_state
...
def _upsert_player(connection: Any, player: IngestPlayer, now: int) -> Set[int]:
    existing = connection.execute(
        'SELECT name,initial,room_label,avatar_url,public_visible '
    ...
    changed = (
        ...
        or bool(existing['public_visible']) != player.public_visible
    ...
    connection.execute(
        'INSERT INTO players('
        'player_id,name,initial,room_label,avatar_url,public_visible,updated_at'
...
def _recompute_ratings(connection: Any, player_ids: Iterable[int]) -> int:
    ...
    all_rows = connection.execute(
        ...
        'exact_fingerprint,stats_eligible,duplicate_of_match_id '
    ...
    for player_id, player_group in groupby(
        ...
        for row in player_group:
            if (
                not bool(row['stats_eligible'])
                or row['duplicate_of_match_id'] is not None
            ):
                continue
...
def apply_ingest_batch(
    ...
    try:
        ...
        previous = connection.execute(
            'SELECT payload_sha256,match_count,removed_match_count,published '
        ...
        if previous is not None:
            ...
            return {
                ...
                'ratingEventCount': 0,
                'published': bool(previous['published']),
            }
        ...
        active_revision = (
            0
            if active is None or active['source_revision'] is None
            else int(active['source_revision'])
        )
        if batch.publish and active_revision > batch.source_revision:
            raise ValueError('dashboard cache already contains a newer source revision')
        if not batch.publish and active_revision > 0:
            raise ValueError('unpublished cache chunks require an empty audience state')
        if batch.reset:
            if active_revision > 0:
                raise ValueError('cache reset requires an empty audience state')
            connection.execute('DELETE FROM matches')
            connection.execute('DELETE FROM players')
            connection.execute('DELETE FROM removed_matches')
            connection.execute('DELETE FROM ingestion_batches')
        ...
        players_removed = _delete_unreferenced_players(
            connection, tuple(current_player_ids)
        )
        content_changed = bool(
            batch.reset
            or changed_player_ids
            or changed_match_ids
            or removed_changed
            or players_removed
        )
        if batch.publish:
            if active_revision > 0 and not content_changed:
                connection.execute(
                    'UPDATE dashboard_audience_state SET source_revision=?, '
                    'published_at=?',
                    (batch.source_revision, now),
                )
            else:
        ...
        connection.execute(
            ...
            'removed_match_count,source_revision,published,applied_at'
            ') VALUES(?,?,?,?,?,?,?,?)',
            (
                idempotency_key,
                payload_sha256,
...
def _rows_matches(
    ...
    *,
    ...
    for player in connection.execute(
        'SELECT player_id,name,initial,room_label,avatar_url,public_visible '
        'FROM players '
        ...
        players[player_id] = {
            ...
            'publicVisible': bool(player['public_visible']),
        }
    ...
    for live in connection.execute(
        'SELECT player_id,room_id,title,started_at FROM player_live_rooms '
        'WHERE player_id IN ('
        + player_placeholders
        + ') ORDER BY player_id,started_at DESC,room_id',
        player_ids,
    ).fetchall():
        players[int(live['player_id'])]['liveRooms'].append(
            {
                'roomId': int(live['room_id']),
                'title': str(live['title']),
                'startedAt': str(live['started_at']),
            }
        )
    ...
    for row in rows:
        ...
        if row['replay_url'] is not None and (
            owner_view or str(row['replay_access']) == 'public'
        ):
            replay_candidates[match_id] = {
                'replay': {
                    'kind': str(row['replay_kind']),
                    'url': str(row['replay_url']),
                }
            }
        values.append(
            {
    ...
    return resolve_match_replays(
        database_target, values, replay_candidates, owner_view=owner_view
    )
...
def list_matches(
    ...
    *,
    ...
    parameters: List[Any] = []
    from_clause = (
        ' FROM matches JOIN players visible_player '
        'ON visible_player.player_id=matches.player_id'
    )
    if not owner_view:
        conditions += ' AND visible_player.public_visible=1'
    ...
    try:
        ...
        total_row = connection.execute(
            'SELECT COUNT(*)' + from_clause + conditions, parameters
        ).fetchone()
        ...
        rows = connection.execute(
            ...
            + from_clause
            + conditions
            + ' ORDER BY matches.played_at_epoch DESC,'
            'matches.source_match_id DESC LIMIT ? OFFSET ?',
    ...
    *,
    ...
    try:
        row = connection.execute(
            'SELECT matches.* FROM matches JOIN players visible_player '
            'ON visible_player.player_id=matches.player_id '
            'WHERE matches.source_match_id=?'
            + ('' if owner_view else ' AND visible_player.public_visible=1'),
            (match_id,),
        ).fetchone()
        if row is None:
    ...
    *,
    ...
    try:
        row = connection.execute(
            ...
            "COALESCE(SUM(CASE matches.result WHEN 'W' THEN 1 ELSE 0 END),0) "
            'AS wins,COUNT(DISTINCT matches.player_id) AS players,'
            'COALESCE(ROUND(AVG(matches.duration_seconds)),0) '
            'AS average_duration,COALESCE(SUM(CASE '
            'WHEN matches.replay_url IS NOT NULL '
            + ('' if owner_view else "AND matches.replay_access='public' ")
            + 'THEN 1 ELSE 0 END),0) AS replays'
            + from_clause
            + conditions,
            parameters,
        ).fetchone()
