...
| 数据同步 | 群晖 NAS worker | API 对局底账；OSS `data/match-images/**` | 秒级监听、2 秒合并、内容变化时写入、失败重试 |
...
1. 使用只读账号从移动云 PostgreSQL 的 `core` schema 提取稳定玩家、别名和符合条件的规范化对局。
2. 与本地已确认水位比较；内容没有变化时跳过写入。
3. 压缩新增或变化的结算图并写入 OSS。
4. 把带幂等键的批次写入 API；失败批次保留在 outbox 中重试。
5. API 在一个事务中更新对局底账，按直播时间重算段位、榜单、英雄统计和当天趋势。

容器启动后会立即校验；之后每秒只读一次数据库变更版本号，变化后等待 2 秒合并
同一批写入。同步失败后默认 60 秒重试，且每天执行一次完整兜底校验。
API 确认前不会推进本地水位；超时重试继续使用同一幂等键。`core` 与 API 使用的
`public` schema 位于同一个 `blrec_dashboard` 数据库，但账号和权限隔离；Publisher
只负责把规范化公开数据写入 API，不能修改 `core`。数据库在每次代码部署前都会
生成并验证在线备份。完整对局由分页接口提供，首页
