...
describe('public guide pages', () => {
  ...
  it('explains the leaderboard sample boundary and rating model', () => {
    ...
    expect(page.textContent).toContain('10 段银约从 8 段银开始');
    ...
    expect(page.textContent).not.toContain('内部值');
    ...
    expect(page.textContent).toContain('历史数据正在持续同步');
    expect(page.textContent).toContain('当前收录最多');
