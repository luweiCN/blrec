        ...
        def replace(connection: sqlite3.Connection) -> int:
            ...
            if row is None:
                connection.execute(
                    ...
                    'id,uid,display_name,credential_ciphertext,credential_version,'
                    'key_id,state,created_at,updated_at) '
                    "VALUES(?,?,?,?,?,?,'active',?,?)",
                    (
                        account_id,
                        account_uid,
                ...
                cursor = connection.execute(
                    'UPDATE bili_accounts SET display_name=?,credential_ciphertext=?,'
                    "credential_version=?,key_id=?,state='active',pause_reason=NULL,"
